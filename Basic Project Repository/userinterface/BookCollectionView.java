...
public class BookCollectionView extends View
{
    ...
    private VBox createFormContent()
    {
        ...
        TableColumn publishYearColumn = new TableColumn("Publish Year") ;
        publishYearColumn.setMinWidth(100);
        publishYearColumn.setCellValueFactory(
        ...
        tableOfBooks.getColumns().addAll(bookIdColumn,
                bookTitleColumn, authorColumn, publishYearColumn, statusColumn);
    ...
    //----------------------------------------------------------
    public void clearTable(){tableOfBooks.getItems().clear();}

    //----------------------------------------------------------
    public void updateTable(BookCollection bookCollection){
        tableOfBooks.getItems().clear();

        Vector<Book> books = (Vector<Book>) bookCollection.getState("Books");
        System.out.println("Test: " + books.size());

        for(Book book: books){
            Vector<String> entry = book.getEntryListView();
            BookTableModel model = new BookTableModel(entry);
            tableOfBooks.getItems().add(model);
        }
    }
